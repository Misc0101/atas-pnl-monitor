...
- **夏普比率** — 年化夏普显示在概览行，与毛盈亏、净盈亏等指标并排；计算公式：`(日均净盈亏 / 样本标准差) × √252`，无风险利率取 0；当前筛选不足 2 个交易日时自动隐藏
...
| `ATAS PnL Monitor Setup 1.2.0.exe` | NSIS 安装包，自动创建桌面快捷方式 |
...
| `ATAS PnL Monitor Setup 1.2.0.exe` | NSIS 安装包 |
